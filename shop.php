...
if (isset($_POST['search_term']) && !empty(trim($_POST['search_term']))) {
    ...
    $relt = mysqli_query($conn, "SELECT * FROM product WHERE product_name LIKE '%$search%' OR category LIKE '%$search%' OR discription LIKE '%$search%'");
} else {
    $relt = mysqli_query($conn, "SELECT * FROM product ORDER BY id DESC");
}
                    ...
                    <div class="row g-4">
                        <div class="col-lg-3">
                            <div class="row g-4">
                                <div class="col-lg-12">
                                    <div class="mb-3">
                                        ...
                                        <ul class="list-unstyled fruite-categorie">
                                            <?php
                                            $cate = mysqli_query($conn, "SELECT DISTINCT category FROM product ");
                                            while ($catefetch = mysqli_fetch_assoc($cate)) {
                                                $catName = $catefetch['category'];

                                                // Count how many products in this category
                                                $countQuery = "SELECT COUNT(*) as total FROM product WHERE category = '$catName'";
                                                $countResult = mysqli_query($conn, $countQuery);
                                                $countData = mysqli_fetch_assoc($countResult);
                                                $productCount = $countData['total'];
                                            ?>
                                                <li>
                                                    <div class="d-flex justify-content-between fruite-name">
                                                        <a href="#"><i class="fas fa-apple-alt me-2"></i><?php echo $catName; ?></a>
                                ...
                                <!-- ---form for sorting by price -->
                                <div class="col-lg-12">
                                    <form method="GET" action="">
                                        <div class="mb-3">
                                            <h4 class="mb-2">Price</h4>
                                            <input type="range" class="form-range w-100" id="rangeInput" name="maxPrice" min="0" max="500" value="<?php echo isset($_GET['maxPrice']) ? $_GET['maxPrice'] : 0; ?>" oninput="amount.value = rangeInput.value">
                                            <output id="amount"><?php echo isset($_GET['maxPrice']) ? $_GET['maxPrice'] : 0; ?></output>
                                        </div>
                                ...
                                <?php
                                $priceLimit = isset($_GET['maxPrice']) ? (int)$_GET['maxPrice'] : 500;

                                $sql = "SELECT * FROM product WHERE price <= $priceLimit";
                        ...
                        <div class="col-lg-9">
                            <div class="row g-4 justify-content-center">
                                ...
                                if (mysqli_num_rows($relt) > 0) {
                                    while ($rowrelt = mysqli_fetch_assoc($relt)) {
                                        // your card layout
                                ?>
                                        <div class="col-md-6 col-lg-6 col-xl-4">
                                            <form action="?" method="POST">
                                                <div class="rounded position-relative fruite-item">
                                                    <a href="shop-detail.php?id=<?php echo $rowrelt['id']; ?>">
                                                        <div class="fruite-img">
                                                            <img style="height: 200px;" src="img/<?php echo $rowrelt['product_img']; ?>" class="img-fluid rounded-top" alt="<?php echo $rowrelt['product_name']; ?>">
                                                        </div>
                                                    </a>
                                                    <div class="text-white bg-secondary px-3 py-1 rounded position-absolute" style="top: 10px; left: 10px;"><?php echo $rowrelt['category'] ?></div>
                                                    <div class="p-4 border border-secondary border-top-0 rounded-bottom">
                                                        <h4><?php echo $rowrelt['product_name']; ?></h4>
                                                        <p><?php echo $rowrelt['discription']; ?></p>
                                                        <input type="hidden" name="product_id" value="<?php echo $rowrelt['id']; ?>">
                                                        <input type="hidden" name="productname" value="<?php echo $rowrelt['product_name']; ?>">
                                                        <input type="hidden" name="price" value="<?php echo $rowrelt['price']; ?>">
                                                        <input type="hidden" name="productimg" value="<?php echo $rowrelt['product_img']; ?>">
                                                        <div class="d-flex justify-content-between flex-lg-wrap">
                                                            <p class="text-dark fs-5 fw-bold mb-0">Rs <?php echo $rowrelt['price']; ?> / kg</p>
                                ...
                                <?php
                                    ...
                                    echo "<p class='text-danger mt-4'>❌ No products found for '<strong>" . htmlspecialchars($_POST['search_term']) . "</strong>'</p>";
                                }
                                ?>
